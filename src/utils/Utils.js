...
Date.prototype.format = function (formatReg) {
...
function getTimestamp(dateStr, isUTC = true) {
    let _date = dateStr;
    if(!isUTC) {
        if(_date && _date.split(' ').length === 1) {
    ...
    return parseInt(_date.getTime() / 1000, 10);
}
function getDateStr(timestamp, format = 'yyyy-MM-dd') {
    let _timestamp = timestamp || Date.now();
    if((parseInt(timestamp, 10) + '').length === 10) {
        _timestamp = parseFloat(timestamp) * 1000;
    }
    return new Date(_timestamp).format(format);
}
...
function judgeTextLength(str, length = 8) {
    try {
        if (str === 0) {
            return str;
        }
